...
export class SiteGeneratorController {
  ...
  async generateSite(
    ...
    this.logger.log(
      `[AI Generator] Gerando site com prompt: ${body.prompt.substring(0, 100)}...`,
      'SiteGeneratorController',
    );

    try {
      const result = await this.siteGeneratorService.generateSite(
        body.prompt,
        body.options || {},
      );
      this.logger.log(
        `[AI Generator] Resultado: success=${result.success}`,
        'SiteGeneratorController',
      );
      return result;
    } catch (error: any) {
      this.logger.error(
        `[AI Generator] Erro: ${error.message}`,
        error.stack,
        'SiteGeneratorController',
      );
      return {
  ...
  async refineSection(
    ...
    this.logger.log(
      `[AI Generator] Refinando seção: ${body.section.type}`,
      'SiteGeneratorController',
    );
    ...
    try {
      ...
      this.logger.error(
        `[AI Generator] Erro ao refinar: ${error.message}`,
        error.stack,
        'SiteGeneratorController',
      );
      return {
  ...
  async generatePatches(
    ...
    this.logger.log(
      `[AI Patcher] Gerando patches: ${body.instruction.substring(0, 100)}...`,
      'SiteGeneratorController',
    );
    ...
    try {
      ...
      this.logger.log(
        `[AI Patcher] Resultado: ${result.patches?.length || 0} patches em ${result.processingTime}ms`,
        'SiteGeneratorController',
      );
      return result;
    } catch (error: any) {
      this.logger.error(
        `[AI Patcher] Erro: ${error.message}`,
        error.stack,
        'SiteGeneratorController',
      );
      return {
